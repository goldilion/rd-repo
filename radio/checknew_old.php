...
//include("includes/db_connection.php");
    ...
    if(isset($_GET['catkey'])) {
        ...
        //$qtotals = "SELECT count(id) as total FROM rtbl_radio_new WHERE category_id = '".$catkey."'";
        $qchannel = "SELECT id, category_id, radio_name, radio_image, radio_url, note, is_deleted FROM rtbl_radio_new WHERE category_id = '".$catkey."' LIMIT $startRow, $perpage";
...
//echo $start.'-'.$totalPage.'-'.$totalRow.'-'.$perpage;
    ...
    showPagination('rtbl_radio_new', $catkey);
        ...
        */
        //echo '<tr>';
        echo '<tr><td>id</td><td>cat id</td><td>name</td><td>url</td><td>station id</td><td>is deleted</td></tr>';
        if($arrChannels){
          $no = 0;
           while ($link = mysql_fetch_assoc($arrChannels)){
                if ($startcheck) {
                    ...
                    //$id[] = $link['id'];
                    //$url[] = $f;
                    //mysql_query("UPDATE rtbl_radio_new SET radio_url = '.$f.' WHERE id = $link['id']");
                    $sqls = 'UPDATE rtbl_radio_new SET radio_url = "'.$f.'" WHERE id = '.$link[id].' AND (radio_url = "#STATUS: 400" OR radio_url = "")';
                    mysql_query($sqls);
                    echo $f;
                    echo '<br />';
                    /*
           ...
           // echo 'Gagal: '.$impFailed;
            ...
            //$insert = implode($id, ",");
            //$urlinsert = 
            //mysql_query("UPDATE rtbl_radio_new SET radio_url = $f WHERE id IN ($insert)");
