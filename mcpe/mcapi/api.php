		...
		private function get_posts() {
			...
			if (isset($_GET['api_key'])) {
				...
				//if ($access_key_received == $api_key) {
					...
					$count_total = $this->get_count_result("SELECT COUNT(DISTINCT m.id) FROM tbl_mcpe m WHERE 
								is_deleted = '0' AND game_app_ref_enc = '$ref_key'");

					$query = "SELECT 
								game_id AS channel_id,
								cat_id AS category_id,
								game_title AS channel_name, 
								game_thumbnail AS channel_image, 
								game_url AS channel_url,
								game_desc AS channel_description,
								game_type AS channel_type,
								'' AS category_name
							FROM `tbl_mcpe` 
							WHERE 
								is_deleted = '0' AND game_app_ref_enc = '$ref_key' LIMIT $limit OFFSET $offset";
